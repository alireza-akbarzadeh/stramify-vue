...
**This skill is read-only.** It analyzes the codebase and outputs recommendations. It does NOT create or modify any
files. Users implement the recommendations themselves or ask Codex separately to help build them.

## Output Guidelines

- **Recommend 1-2 of each type**: Don't overwhelm - surface the top 1-2 most valuable automations per category
- **If user asks for a specific type**: Focus only on that type and provide more options (3-5 recommendations)
- **Go beyond the reference lists**: The reference files contain common patterns, but use web search to find
  recommendations specific to the codebase's tools, frameworks, and libraries
...
| **Skills**      | Packaged expertise, workflows, and repeatable tasks (invoked by Codex or user via `/skill-name`) |
...
# Check for existing Codex config
...
Create skills in `.Codex/skills/<name>/SKILL.md`. Some are also available via plugins:
...
| Code style        | **project-conventions**                       | Codex-only |
...
Format recommendations clearly. **Only include 1-2 recommendations per category** - the most valuable ones for this
specific codebase. Skip categories that aren't relevant.
...
**Install**: `Codex mcp add context7`
...
#### [skill name]
**Why**: [specific reason]
**Create**: `.Codex/skills/[name]/SKILL.md`
**Invocation**: User-only / Both / Codex-only
...
**Where**: `.Codex/agents/[name].md`

---

**Want more?** Ask for additional recommendations for any specific category (e.g., "show me more MCP server options"
...
- `user-invocable: false` — Codex-only (for background knowledge)
...
Recommend headless Codex for automated pipelines:

```bash
# Pre-commit hook example
Codex -p "fix lint errors in src/" --allowedTools Edit,Write

# CI pipeline with structured output
Codex -p "<prompt>" --output-format stream-json | your_command
```

### Permissions for Hooks

Configure allowed tools in `.Codex/settings.json`:
