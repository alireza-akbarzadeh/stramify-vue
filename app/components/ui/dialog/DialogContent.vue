...
// Track A motion per the `motion` skill: the open/close transition is CSS
// keyed off Reka's `data-state`, so the exit animation actually runs before
// the node is removed. `motion-v` is for content, not for overlay chrome.
interface DialogContentProperties extends DialogContentProps {
...
const props = defineProps<DialogContentProperties>()
