...
- **`GET /api/watch-later/ids`** — just the saved clip ids, so every bookmark in
  the app can draw itself. Signed out it answers `[]`.
- **`POST /api/watch-later`** `{ clipId }` — save. Idempotent.
- **`DELETE /api/watch-later/[clipId]`** — unsave. Idempotent.

Two surfaces render the queue itself: the **Watch later shelf** on the home page
(ten cards, "See all" → the page) and **`/watch-later`** itself (a grid, up to
sixty).

**Every bookmark in the app writes it.** The filled/hollow bookmark on a card,
the shorts rail's Save, the Save button under the player and the ⋮ menu's "Save
to Watch later" are all the same action, routed through
`composables/useSavedVideos.ts`. Saving raises a toast carrying an Undo; the
second press of a filled bookmark unsaves, optimistically.

That seam is also where **live sessions** peel off to the browser-local
watchlist — see below, and ADR-027 for why the bookmark used to mean something
else entirely.
...
- The **watchlist** (localStorage, `/watchlist`) can't back this: a queue you
  can't reach from your phone isn't a queue. It stays for the two things it's
  still the only answer to — a save made by a signed-out visitor, and a **live
  channel**, which `watch_later` has no room for (see "Clips only" below). It is
  no longer where a clip's bookmark goes; ADR-027 has the story.
...
saved; the stream itself can't, which is why a live card's bookmark falls
through to the on-device list and says "watchlist" in its label rather than
"Watch later".
...
Everything lives under the `['watch-later']` prefix, in two shapes:

- `['watch-later', 'list', 10]` — the home rail
- `['watch-later', 'list', 60]` — the page
- `['watch-later', 'ids']` — every bookmark on every page, one entry between them

The two shapes are kept apart on purpose. A `setQueriesData` over the bare
prefix would hand the id array to a callback written for cards, so the
optimistic patches target `'list'` and `'ids'` separately; invalidation still
uses the prefix and covers both.

Both writes patch `'ids'` optimistically — the bookmark that triggered them is
on screen and has to fill or empty at once — and only *invalidate* the lists,
because a card assembled client-side would be a guess at `addedAt` and the
channel avatar. Removing does patch every cached list too, so a card dismissed
on the rail is already gone when you walk to the page. `staleTime: 0`
throughout — saving a video and coming back to a shelf that doesn't show it
reads as a dropped save.
...
- **`POST` with an unknown clip id** is a 404, not a 500 — the id is checked
  ...
  violation.
- **Signed out**, the bookmark shows a toast ("Log in to save videos for later")
  rather than failing at the endpoint; the rail and page render their sign-in
  states. The bookmark itself always draws hollow, because `['watch-later',
  'ids']` is gated on the session and answers `[]` anyway.
- **Clips saved to the old localStorage watchlist before ADR-027** are not
  migrated. They stay visible under `/watchlist`'s "Saved clips", but their
  bookmark on a card now reflects `watch_later`, so an old entry reads as
  unsaved. Pressing it saves it properly.
...
- ADR-027 in [DECISIONS.md](./DECISIONS.md) — why the bookmark now means this
  queue, and why live channels still don't fit in it.
